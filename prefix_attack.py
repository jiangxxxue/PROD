...
def sample_code_from_llm(args, prompt, prefix_token_id, model, tokenizer):
    completions = []
    input_ids = [tokenizer.bos_token_id] + tokenizer.encode(prompt, add_special_tokens=False, verbose=False) + prefix_token_id
...
def load_model_tokenizer(args, model_name, model_path):
    ...
    generate_code_fn = lambda args, prompt, prefix_token_id: sample_code_from_llm(
        args, prompt, prefix_token_id, model, tokenizer
    )
...
def generate_code_for_tasks(args, except_tasks, save_file):
    ...
    for i in tqdm(range(len(dataset))):
        ...
        if args.k > 0:
            # k prefix token
            ground_truth_token_id = tokenizer.encode(ground_truth, add_special_tokens=False, verbose=False)
            ground_truth_token_len = len(ground_truth_token_id)
            prefix_len = args.k if args.k < ground_truth_token_len else (ground_truth_token_len-1)
            prefix_token_id = ground_truth_token_id[:prefix_len]
            suffix_token_id = ground_truth_token_id[prefix_len:]
            suffix_token = tokenizer.decode(suffix_token_id)
        else:
            prefix_token_id = []
            suffix_token = tokenizer.tokenize(ground_truth)

        # generate code and write to file
        BLEU = 0
        for completion in generate_code_fn(args, prompt, prefix_token_id):
            output ={
                    "task_id": task_id,
                    "prompt": prompt,
            ...
            BLEU += sentence_bleu([tokenizer.tokenize(suffix_token)], tokenizer.tokenize(completion))
...
def main(args):
    ...
    save_file = os.path.join(
        args.output_dir,
        f"{dataset}_{model_name}_temp{args.temperature}_topp{args.topp}_topk{args.topk}_samples{args.num_samples}_{args.few_shot}shot_{args.output_file_suffix}_prefix_token_{args.k}.jsonl",
    )
    print(save_file)
    except_tasks = []
